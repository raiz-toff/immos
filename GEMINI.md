...
For Gemini CLI, prefix commands with `@` if using the function-call interface,
or speak them naturally as requests if using the conversational interface.
...
- File read/write functions map directly to the permission matrix in `docs/DATA_CONTRACT.md`
- Do not invoke file-write functions on READ ONLY paths (see DATA_CONTRACT.md)
